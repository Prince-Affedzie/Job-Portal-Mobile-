...
const BIDDING_CONFIG = {
  'open-bid': {
    ...
    btnIcon: 'pricetag', btnText: 'Place a Bid', sentText: 'Bid Sent!',
  },
  negotiation: {
    icon: 'chatbubble-ellipses-outline', label: 'Open to Negotiation', color: C.gold, bg: C.goldLight,
    btnIcon: 'chatbubbles', btnText: 'Start Negotiation', sentText: 'Offer Sent!',
  },
  fixed: {
    icon: 'lock-closed-outline', label: 'Fixed Budget', color: C.green, bg: C.greenLight,
    btnIcon: 'hand-right', btnText: 'Show Interest', sentText: 'Interest Sent!',
  },
};
...
// ─── Header Options Menu ──────────────────────────────────────────────────────
function HeaderOptionsMenu({ visible, onClose, onReport, onSave }) {
  if (!visible) return null;
  return (
    <TouchableOpacity style={hm.backdrop} activeOpacity={1} onPress={onClose}>
      <View style={hm.menu}>
        {/* Report */}
        <TouchableOpacity style={hm.item} onPress={() => { onClose(); onReport(); }}>
        ...
        {/* Save (non‑functional placeholder) */}
        <TouchableOpacity style={hm.item} onPress={onClose}>
          <View style={[hm.iconBox, { backgroundColor: C.primaryGlow }]}>
            <Ionicons name="bookmark-outline" size={18} color={C.primaryMid} />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={hm.itemText}>Save</Text>
            <Text style={hm.itemSub}>Coming soon</Text>
          </View>
        </TouchableOpacity>
      </View>
    </TouchableOpacity>
...
const TaskDetailsScreen = ({ route, navigation }) => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Header title="Task Details" showBackButton showNotifications={false}/>

      {/* ── Header options (⋮) ─────────────────────────────────────────── */}
      <TouchableOpacity
        style={[styles.headerOptionsBtn, { top: insets.top + 8 }]}
      ...
      <HeaderOptionsMenu
        ...
        onSave={() => Alert.alert('Coming soon', 'Save feature will be available soon.')}
      />
      ...
      >
        ...
        <View style={styles.content}>
          ...
          {/* Client — without the in‑card report button */}
          ...
          <View style={styles.safetyCard}>
            ...
            <TouchableOpacity style={styles.safetyLearnMore} onPress={() => setShowScamAlert(true)}>
      ...
      <View style={[styles.footer, { paddingBottom: insets.bottom + 12 }]}>
        ...
        >
          {applying ? (
            ...
            <>
              <Ionicons name={applyClicked ? 'checkmark-circle' : biddingCfg.btnIcon} size={21} color={C.white} />
      ...
      <BidModal visible={showBidModal} onClose={() => setShowBidModal(false)} bidData={bidData} setBidData={setBidData} onSubmit={submitBid} isProcessing={applying} />
      <NegotiationModal visible={showNegotiationModal} onClose={() => setShowNegotiationModal(false)} negotiationData={negotiationData} setNegotiationData={setNegotiationData} onSubmit={submitNegotiation} isProcessing={applying} />
...
const styles = StyleSheet.create({
  ...
  safetyLearnMore: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingTop: 10, borderTopWidth: 1, borderTopColor: '#FDE68A' },
  safetyLearnMoreText: { fontSize: 13, fontWeight: '700', color: C.primaryMid },

  // ── Footer ──
  footer: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: C.surface, paddingHorizontal: 16, paddingTop: 12, borderTopWidth: 1, borderTopColor: C.border, shadowColor: '#000', shadowOffset: { width: 0, height: -3 }, shadowOpacity: 0.08, shadowRadius: 10, elevation: 10 },
  applyBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: C.primaryMid, paddingVertical: 16, borderRadius: 14, gap: 8, shadowColor: C.primaryMid, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.28, shadowRadius: 10, elevation: 6 },
  applyBtnDisabled: { backgroundColor: C.textMuted, shadowOpacity: 0, elevation: 0 },
  applyBtnText: { color: C.white, fontSize: 16, fontWeight: '700', letterSpacing: -0.2 },
});
...
export const scamStyles = StyleSheet.create({
  scamAlertOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.55)', justifyContent: 'center', alignItems: 'center', padding: 20 },
  scamAlertContent: { backgroundColor: '#FFFFFF', borderRadius: 20, padding: 24, width: '100%' },
  scamAlertTitle: { fontSize: 18, fontWeight: '800', color: '#DC2626', marginBottom: 16, textAlign: 'center' },
  scamAlertList: { marginBottom: 24 },
  scamAlertItem: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: 12, gap: 10 },
  scamAlertText: { fontSize: 14, color: '#475569', flex: 1, lineHeight: 20 },
  scamAlertButton: { backgroundColor: '#1A56DB', padding: 16, borderRadius: 14, alignItems: 'center' },
  scamAlertButtonText: { color: '#FFFFFF', fontWeight: '700', fontSize: 15 },
});
